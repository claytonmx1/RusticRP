...
local cropstateMethods = {
    plant = function(instance, location, soil, stage)
        if onServer then
            ...
            function(id)
                ...
                TriggerClientEvent('rustic_plantgrow:planted',-1, id, location, stage)
    ...
    update = function(instance, id, stage)
        ...
        if onServer then
            ...
            MySQL.Async.execute("UPDATE `uteknark` SET `stage` = @stage WHERE `id` = @id LIMIT 1;",
            {
                ...
                TriggerClientEvent('rustic_plantgrow:update', -1, id, stage)
    ...
    remove = function(instance, id, withPyro)
        ...
        if onServer then
            ...
            { ['@id'] = id },
            function()
                TriggerClientEvent('rustic_plantgrow:removePlant', -1, id)
                if withPyro then
                    TriggerClientEvent('rustic_plantgrow:pyromaniac', -1, location)
                end
    ...
    bulkData = function(instance, target)
        if onServer then
            ...
            TriggerClientEvent('rustic_plantgrow:bulk_data', target, forest)
        else
            TriggerServerEvent('rustic_plantgrow:request_data')
        end
    end,
}
...
if onServer then
    RegisterNetEvent('rustic_plantgrow:request_data')
    AddEventHandler ('rustic_plantgrow:request_data', function()
        cropstate:bulkData(source)
    end)
    
    RegisterNetEvent('rustic_plantgrow:remove')
    AddEventHandler ('rustic_plantgrow:remove', function(plantID, nearLocation)
        local src = source
        ...
        if plant then
            ...
            TriggerClientEvent('rustic_plantgrow:remove', src, plantID)
        end
    end)

    RegisterNetEvent('rustic_plantgrow:frob')
    AddEventHandler ('rustic_plantgrow:frob', function(plantID, nearLocation)
        local src = source
        local plant = cropstate.index[plantID]
        if plant then
            local plantLocation = plant.bounds.location
    ...
    RegisterNetEvent('rustic_plantgrow:bulk_data')
    AddEventHandler ('rustic_plantgrow:bulk_data', function(forest)
    ...
    RegisterNetEvent('rustic_plantgrow:planted')
    AddEventHandler ('rustic_plantgrow:planted', function(id, location, stage)
        cropstate:import(id, location, stage)
    end)
    
    RegisterNetEvent('rustic_plantgrow:update')
    AddEventHandler ('rustic_plantgrow:update', function(plantID, stage)
        cropstate:update(plantID, stage)
    end)

    RegisterNetEvent('rustic_plantgrow:removePlant')
    AddEventHandler ('rustic_plantgrow:removePlant', function(plantID)
        cropstate:remove(plantID)
    end)
end
