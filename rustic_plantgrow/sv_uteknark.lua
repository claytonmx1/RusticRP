...
function makeToast(target, subject, message)
    TriggerClientEvent('rustic_plantgrow:make_toast', target, subject, message)
end
...
function doScenario(who, what, where)
    ...
    TriggerClientEvent('rustic_plantgrow:do', who, what, where)
end

RegisterNetEvent('rustic_plantgrow:success_plant')
AddEventHandler ('rustic_plantgrow:success_plant', function(location, soil)
    local src = source
...
RegisterNetEvent('rustic_plantgrow:log')
AddEventHandler ('rustic_plantgrow:log',function(...)
    local src = source
    log(src,GetPlayerName(src),...)
end)

RegisterNetEvent('rustic_plantgrow:test_forest')
AddEventHandler ('rustic_plantgrow:test_forest',function(forest)
    local src = source
                ...
                ESX.RegisterUsableItem(Config.Items.Seed, function(source)
                    ...
                    if now > last + (Config.ActionTime/1000) then
                        if HasItem(source, Config.Items.Seed) then
                            TriggerClientEvent('rustic_plantgrow:attempt_plant', source)
...
local commands = {
    debug = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('rustic_plantgrow:toggle_debug', source)
        end
    end,
    ...
    forest = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('rustic_plantgrow:test_forest', source, count, randomStage)
    ...
    groundmat = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('rustic_plantgrow:groundmat', source)
        end
    end,
    pyro = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('rustic_plantgrow:pyromaniac', source)
        end
    end,
}
