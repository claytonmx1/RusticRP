...
function serverlog(...)
    TriggerServerEvent('rustic_plantgrow:log',...)
end

RegisterNetEvent('rustic_plantgrow:make_toast')
AddEventHandler ('rustic_plantgrow:make_toast', function(subject,message)
    -- OLD NOTIFITCATION STUFF makeToast(subject, message)
    exports['mythic_notify']:SendAlert('inform', message, 6500)
end)
...
function getPlantingLocation(visible)
    ...
    if hit == 1 then
        ...
        if Config.Soil[material] then
            ...
            if flatEnough(surfaceNormal) then
                ...
                if plantDistance <= Config.Distance.Interact then
                    ...
                    if #hits > 0 then
                        ...
                        return false, 'Plant Needs More Room', hitLocation, surfaceNormal, material
                    else
                        if visible then
                    ...
                    return false, 'Too Far Away', hitLocation, surfaceNormal, material
                end
            else
                if visible then
                ...
                return false, 'Too Steep To Plant', hitLocation, surfaceNormal, material
            end
        else
            if visible then
            ...
            return false, 'Soil Is Shitty', hitLocation, surfaceNormal, material
        end
    else
        if visible then
...
RegisterNetEvent('rustic_plantgrow:do')
AddEventHandler ('rustic_plantgrow:do', function(scenarioName, location)
...
RegisterNetEvent('rustic_plantgrow:attempt_plant')
AddEventHandler ('rustic_plantgrow:attempt_plant', function()
    local plantable, message, location, _, soil = getPlantingLocation()
    if plantable then
        TriggerServerEvent('rustic_plantgrow:success_plant', location, soil)
        lastAction = GetGameTimer()
    else
        --OLD NOTIFY SHIT makeToast(_U('planting_text'), _U(message))
        exports['mythic_notify']:SendAlert('inform', message, 6500)
    end
end)
                ...
                if closestDistance <= Config.Distance.Interact then
                    ...
                    if now >= lastAction + Config.ActionTime then
                        if IsDisabledControlJustPressed(0, 44) then
                            ...
                            TriggerServerEvent('rustic_plantgrow:remove', closestPlant.id, myLocation)
                        else
                            if stage.interact then
                                interactHelp(closestPlant.stage, _U(stage.label))
                                if IsControlJustPressed(0, 38) then
                                    lastAction = now
                                    TriggerServerEvent('rustic_plantgrow:frob', closestPlant.id, myLocation)
                                end
                            else
...
RegisterNetEvent('rustic_plantgrow:toggle_debug')
AddEventHandler ('rustic_plantgrow:toggle_debug', function()
...
RegisterNetEvent('rustic_plantgrow:pyromaniac')
AddEventHandler ('rustic_plantgrow:pyromaniac',function(location)
...
RegisterNetEvent('rustic_plantgrow:groundmat')
AddEventHandler ('rustic_plantgrow:groundmat', function()
    local plantable, message, where, normal, material = getPlantingLocation(true)
...
RegisterNetEvent('rustic_plantgrow:test_forest')
AddEventHandler ('rustic_plantgrow:test_forest',function(count, randomStage)
    ...
    TriggerServerEvent('rustic_plantgrow:test_forest', forest)
end)
