...
RegisterNetEvent("rustic_properties:Option")
AddEventHandler("rustic_properties:Option", function(option, coords, distanceAmount, cb)
	cb(Menu.Option(option, coords, distanceAmount))
end)

RegisterNetEvent("rustic_properties:Update")
AddEventHandler("rustic_properties:Update", function()
	Menu.updateSelection()
end)

RegisterNetEvent("rustic_properties:UpdateOption")
AddEventHandler("rustic_properties:UpdateOption", function()
    Menu.UpdateOption() 
end)
...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	TriggerServerEvent('rustic_properties:updateSpawned')
	ESX.TriggerServerCallback('rustic_properties:GetEmpty', function(result)
...
properties = {
	{tier = "low", locations = {
		...
		{label = "Mirror Park 1", name = "MirrorPark1", enter = {x = 1303.05, y = -527.83, z = 71.46, h = 343.33}, inside = {x = 190.51, y = -1723.2, z = 20.92, h = 279.03}, exit = {x = 191.09, y =-1724.34, z = 22.96}, outside = {1303.05, y = -527.83, z = 71.46, h = 343.33}, empty = true, owner = false, locked = false, haskey = false, max = 2, price = 350000}
	}},
}
...
function checkOwned()
	ESX.TriggerServerCallback('rustic_properties:GetOwned', function(result)
		for i = 1, #result, 1 do
...
Citizen.CreateThread(function()
	...
	while true do
		...
		elseif IsControlJustReleased(0, 46) then
			TriggerEvent('rustic_properties:UpdateOption')
			menu = false
		end
		...
		if(menu) then
			for x = 1, #props, 1 do
				...
				if distance <= 2.5 then
					if props[x].locked == 0 or props[x].owner == true then
							TriggerEvent("rustic_properties:Option", "Deposit Item", {props[x].x, props[x].y, props[x].z}, 2.0, function(cb)
							if(cb) then
								OpenPlayerInventoryMenu()
							else
							end
						end)
						TriggerEvent("rustic_properties:Option", "Remove Item", {props[x].x, props[x].y, props[x].z}, 2.0, function(cb)
							if(cb) then
					...
					if props[x].owner == true then
						if props[x].locked == 1 then
							TriggerEvent("rustic_properties:Option", "Unlock", {props[x].x, props[x].y, props[x].z}, 2.0, function(cb)
								if(cb) then
									TriggerServerEvent('rustic_properties:UpdateStorageLock', {x = props[x].x, y = props[x].y, z = props[x].z}, 0)
									props[x].locked = 0
								else
								end
							end)
						elseif props[x].locked == 0 then
							TriggerEvent("rustic_properties:Option", "Lock", {props[x].x, props[x].y, props[x].z}, 2.0, function(cb)
								if(cb) then
									TriggerServerEvent('rustic_properties:UpdateStorageLock', {x = props[x].x, y = props[x].y, z = props[x].z}, 1)
									props[x].locked = 1
								else
								end
							end)
				...
				for j = 1, #properties[i].locations do
					...
					if properties[i].locations[j].owner or properties[i].locations[j].locked == false and properties[i].locations[j].empty == false then
						if distanceEnter <= 5.0 then
							TriggerEvent("rustic_properties:Option", "Enter", {properties[i].locations[j].enter.x, properties[i].locations[j].enter.y, properties[i].locations[j].enter.z}, 2.0, function(cb)
								if(cb) then
									inside = true
					...
					if properties[i].locations[j].owner then				
						if distanceEnter <= 5.0 then
							if properties[i].locations[j].locked == true then 
								TriggerEvent("rustic_properties:Option", "Unlock Door ", {properties[i].locations[j].enter.x, properties[i].locations[j].enter.y, properties[i].locations[j].enter.z}, 2.0, function(cb)
									if(cb) then
										TriggerEvent("dooranim")
										Wait(750)
										properties[i].locations[j].locked = false
										TriggerServerEvent('rustic_properties:UpdatePropertyLock', properties[i].locations[j].name, 0)
									else
									end
								end)
							elseif properties[i].locations[j].locked == false then
								TriggerEvent("rustic_properties:Option", "Lock Door", {properties[i].locations[j].enter.x, properties[i].locations[j].enter.y, properties[i].locations[j].enter.z}, 2.0, function(cb)
									if(cb) then
										TriggerEvent("dooranim")
										Wait(750)
										properties[i].locations[j].locked = true
										TriggerServerEvent('rustic_properties:UpdatePropertyLock', properties[i].locations[j].name, 1)
									else
									end
								end)
							end
							if properties[i].tier == "low" then
								TriggerEvent("rustic_properties:Option", "Sell house", {properties[i].locations[j].enter.x, properties[i].locations[j].enter.y, properties[i].locations[j].enter.z}, 2.0, function(cb)
									if(cb) then
										menu = false
										TriggerEvent('rustic_properties:UpdateOption')
										...
										if UpdateOnscreenKeyboard() ~= 2 then
											...
											if result == "y" or result == "yes" or result == "ye" then
												TriggerEvent("dooranim")
												Wait(750)
												TriggerServerEvent('rustic_properties:SellProperty', properties[i].locations[j].name)
					...
					if properties[i].locations[j].empty then
						if distanceEnter <= 5.0 then
							TriggerEvent("rustic_properties:Option", "Look at place", {properties[i].locations[j].enter.x, properties[i].locations[j].enter.y, properties[i].locations[j].enter.z}, 2.0, function(cb)
								if(cb) then
									TriggerEvent("dooranim")
									Wait(750)
									DoScreenFadeOut(1)
							...
							TriggerEvent("rustic_properties:Option", "Buy $" .. properties[i].locations[j].price, {properties[i].locations[j].enter.x, properties[i].locations[j].enter.y, properties[i].locations[j].enter.z}, 2.0, function(cb)
								if(cb) then
									TriggerServerEvent('rustic_properties:BuyProperty', properties[i].locations[j].name)
									TriggerEvent('rustic_properties:UpdateOption')
								else
								end
							end)
					...
					if distanceExit <= 3.0 then
						TriggerEvent("rustic_properties:Option", "Exit", {properties[i].locations[j].exit.x, properties[i].locations[j].exit.y, properties[i].locations[j].exit.z}, 2.0, function(cb)
							if(cb) then
								inside = false
								TriggerEvent("dooranim")
								Wait(750)
						...
						if properties[i].locations[j].owner then				
							if properties[i].locations[j].locked == true then 
								TriggerEvent("rustic_properties:Option", "Unlock Door", {properties[i].locations[j].exit.x, properties[i].locations[j].exit.y, properties[i].locations[j].exit.z}, 2.0, function(cb)
									if(cb) then
										TriggerEvent("dooranim")
										Wait(750)
										properties[i].locations[j].locked = false
							...
							elseif properties[i].locations[j].locked == false then
								TriggerEvent("rustic_properties:Option", "Lock Door", {properties[i].locations[j].exit.x, properties[i].locations[j].exit.y, properties[i].locations[j].exit.z}, 2.0, function(cb)
									if(cb) then
										TriggerEvent("dooranim")
										Wait(750)
										properties[i].locations[j].locked = true
			...
			TriggerEvent("rustic_properties:Update")
		end
...
Furniture = {
	{type = "Storage", items = {
		{name = "safe", maxAmount = 1, propName = "p_v_43_safe_s"}
	}},
}
...
RegisterNetEvent('rustic_properties:updateStorageLock')
...
RegisterNetEvent('rustic_properties:deleteObjectsOnSell')
AddEventHandler('rustic_properties:deleteObjectsOnSell', function(propertyName)
	for f,L in pairs (props) do
	...
	for k,v in pairs (props) do
		for q,r in pairs (Furniture) do
			for t,y in pairs (r.items) do
				...
				if distance <= 2 then
					v.selected = true
					TriggerServerEvent('rustic_properties:updateSelected', { x = v.x, y = v.y, z = v.z, r = v.rotation}, y.name, 1)
						...
						for q,r in pairs (Furniture) do
							for t,y in pairs (r.items) do
								if v.selected == true then
									...
									if update == true then
										if v.selected == true then
											TriggerServerEvent('rustic_properties:updateProp', { x = v.x, y = v.y, z = v.z, r = v.rotation}, 0)
											v.selected = false
											update = false
										end
									elseif update == false then
										TriggerServerEvent('rustic_properties:addProp', v.name, { x = v.x, y = v.y, z = v.z, r = v.rotation}, 0, "{}", 0, 0, currentProperty)
										v.selected = false
									end
								end
							end
							...
							for t,y in pairs (r.items) do
								if v.selected == true then
									if update == true then
										...
										v.selected = false
										update = false
										TriggerServerEvent('rustic_properties:updateSelected', { x = v.x, y = v.y, z = v.z, r = v.rotation}, y.name, 0)
									else
										TriggerServerEvent('rustic_properties:addPropBack', v.spawnname)
										drawing = false
										DeleteObject(v.Prop)
...
function checkLock2()
	ESX.TriggerServerCallback('rustic_properties:GetLock', function(result)
		for k, v in pairs(props) do
...
function OpenPlayerInventoryMenu()
	ESX.TriggerServerCallback('rustic_properties:getPlayerInventory', function(inventory)
		for k,v in pairs(properties) do
			for i,j in pairs(v.locations) do
					...
					if distance <= 2 then
						...
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_inventory', {
							...
							if data.current.type == 'item_weapon' then
								menu.close()
								TriggerServerEvent('rustic_properties:putItem', {x = w.x, y = w.y, z = w.z}, data.current.type, data.current.value, data.current.label2, data.current.ammo)
							else
								ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'put_item_count', {
									...
									if quantity == nil then
										...
										menu2.close()
										TriggerServerEvent('rustic_properties:putItem', {x = w.x, y = w.y, z = w.z}, data.current.type, data.current.value, data.current.label2, tonumber(data2.value))
									end
...
function OpenRoomInventoryMenu()
	ESX.TriggerServerCallback('rustic_properties:getPropertyInventory', function(inventory)
		for k,v in pairs(properties) do
			for i,j in pairs(v.locations) do
				for x,w in pairs(props) do
						local coords = GetEntityCoords(PlayerPedId())
						...
						if distance <= 2 then
							...
							ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'room_inventory', {
								...
								if data.current.type == 'item_weapon' then
									...
									TriggerServerEvent('rustic_properties:getItem', {x = w.x, y = w.y, z = w.z}, data.current.type, data.current.value, data.current.index)
									...
									ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'get_item_count', {
										...
										if quantity == nil then
											...
											TriggerServerEvent('rustic_properties:getItem', {x = w.x, y = w.y, z = w.z}, data.current.type, data.current.value, quantity)
											ESX.SetTimeout(300, function()
												OpenRoomInventoryMenu()
											end)
										end
...
Citizen.CreateThread(function()
	...
	---	TriggerServerEvent("rustic_properties:checkifitsbought")
	--end
	for k,v in pairs(properties) do
	for i,j in pairs(v.locations) do
...
--[[
--EVENT TO GET RID OF BOUGHT HOUSES BLIPS

RegisterNetEvent("setownedblips")
AddEventHandler("setownedblips", function(housecords, storedblips)
local propertyName = ""
local wowstoredblips = ""
if #storedblips == 0 then
	print("We dont have any house owners in the city")
		for k,v in pairs(properties) do
			for i,j in pairs(v.locations) do
				if propertyName ~= j.name then 
				...
				AddTextComponentSubstringPlayerName(j.label)
				EndTextCommandSetBlipName(blip)
				TriggerServerEvent("rustic_properties:addingblip", j.name)
			end
		end
	end
end
 ...
 if #storedblips > 0 then
	for i=1, #housecords, 1 do
		for t=1, #storedblips, 1 do
		propertyName = housecords[i].name
		wowstoredblips = storedblips[t].propertyxcord
		print(propertyName)

		for k,v in pairs(properties) do
			for i,j in pairs(v.locations) do
				if propertyName ~= j.name and j.name ~= wowstoredblips then
				print("property does not exist blip needed") 
				...
				if j.name == propertyName and j.name == wowstoredblips then
					print("property exist no blip needed")
				local blip = AddBlipForCoord(j.enter.x, j.enter.y, j.enter.z)
				RemoveBlip(blip)
				end
			end
