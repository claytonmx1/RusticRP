    ...
    MySQL.Async.fetchAll('SELECT * FROM `owned_vehicles` WHERE `owner` = @identifier AND `type` = @type', {
        ...
        if result[1] ~= nil then
            for k, v in pairs(result) do
                ...
                table.insert(vehicles, {plate = v.plate, vehicle = veh, stored = v.stored, health = health, fuel = fuellevel})
            end
            callback(vehicles)
        else
            callback(nil)
...
AddEventHandler('luke_vehiclegarage:SaveVehicle', function(vehicle, health, plate, fuel)
    MySQL.Async.execute('UPDATE `owned_vehicles` SET `vehicle` = @vehicle, `health` = @health, `fuel` = @fuel WHERE `plate` = @plate', {
